...
### 请分析ucore_lab中提供的参考答案，并请在实验报告中说明你的实现与参考答案的区别

1. 练习2的参考答案是通过修改`Makefile`和`gdbinit`来修改调试位置的，而我是仿照`Makefile`中的命令手动执行。这两种方法没有本质上的不同；
2. 练习2的参考答案中，是通过在gdb中使用`x /2i $pc`命令查看当前执行到的汇编指令的。但如上文所述，我所使用的qemu(2.5.0)和gdb(7.11.1)并不能正确解析`$pc`，故需要手动查看；
3. 练习6初始化中断描述符表时，我按照要求将syscall和真正的trap的`istrap`位均置1，而参考答案全都置0；我按要求将syscall的`dpl`置为用户态，而参考答案将所有trap/interruption的`dpl`均置为内核态。此处应为参考答案的疏漏。

### 重要知识点

1. OS启动流程：原理课中讲解的OS启动时执行的三个程序主体：BIOS、bootloader和内核，除BIOS是在ROM实现而无法在实验中涉及外，后两部分在实验中均有较为详细的体现。实验中还涉及了启动扇区和ELF的具体格式、切换到保护模式的具体过程等细节；
2. C函数调用：实验中打印调用栈的练习，涉及了C函数调用的知识点；
3. x86的中断机制：原理课中讲解的x86中断向量、中断描述符的具体设置，在实验中均有体现。但关于特权级切换的部分，要在后续实验中才有涉及。 
